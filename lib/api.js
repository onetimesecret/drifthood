...
// ── Shared Testruns (public, no auth) ──

export async function apiGetSharedTestrun(testrunExtid, signal) {
  // Plain fetch without auth header - public share endpoint
  const resp = await fetch(`/api/share/${testrunExtid}`, { signal });
  assertOk(resp);
  return resp.json();
}

export async function apiSetTestrunPublic(testrunExtid, isPublic) {
  const resp = await apiFetch(`/api/testruns/${testrunExtid}/public`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
