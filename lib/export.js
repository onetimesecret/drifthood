...
export function buildContextLines(format, ctx) {
...
export function buildDriftSummary(r, ctx) {
  ...
  if (r.ignored_paths && r.ignored_paths.length) {
    lines.push(`Ignored (${r.ignored_paths.length}): ${r.ignored_paths.map(p => prettifyPath(p)).join(', ')}`);
  }

  return lines.join('\n');
}

// ── Curl command (for "Copy curl" button in DiffView) ──
...
/**
 * Build a curl command that reproduces the request DriftDetector sent.
 * @param {object} r - comparison result
 * @param {'a'|'b'} [side='b'] - which side to build for
 */
export function buildCurlCommand(r, side = 'b') {
  const resp = side === 'a' ? r.response_a : r.response_b;
  if (!resp?.request_url) return '';

  const parts = ['curl -sk -D-'];
  ...
  if (resp.request_body) {
    const escaped = resp.request_body.replace(/'/g, "'\\''");
    parts.push(`-d '${escaped}'`);
  }

  parts.push(resp.request_url);
...
export function fmtHeaders(headers) {
...
export function buildSideMd(r, side, ctx) {
  const resp = side === 'a' ? r.response_a : r.response_b;
  const memo = side === 'a' ? (ctx?.memoA || '') : (ctx?.memoB || '');
  const label = memo ? `${memo}` : `Environment ${side.toUpperCase()}`;
  let md = `### ${label}\n\n`;
...
export function buildFullMd(r, ctx) {
  ...
  md += `## ${r.method} ${r.path}\n\n`;
  const bothError = (r.response_a.status == null && r.response_a.error || r.response_a.status >= 400)
                  && (r.response_b.status == null && r.response_b.error || r.response_b.status >= 400);
  const resultLabel = r.has_drift ? 'DRIFT' : (bothError ? 'MATCH' : 'OK');
  md += `**Result:** ${resultLabel}\n\n`;
