...
# Validates that all Ruby, Python, TypeScript, JavaScript, Vue, and Svelte files
# have correct header format
#
# Usage:
...
class HeaderValidator
    ...
    if @paths.empty?
      ...
      @paths.flat_map do |path|
        ...
        if full_path.directory?
          ...
          Dir.glob(File.join(path, '**/*.{rb,py,ts,js,vue,svelte}'), base: REPO_ROOT)
  ...
  def validate_javascript_files
    ...
    files_for_glob('src/**/*.js').each do |file_path|
      ...
      @files_found += 1
      validate_javascript_header(full_path, file_path)
    end
  end
  ...
  def validate_svelte_files
    puts 'Checking Svelte files...'

    files_for_glob('src/**/*.svelte').each do |file_path|
      next if skip_file?(file_path)
      ...
      @files_found += 1
      validate_svelte_header(full_path, file_path)
    end
  end
  ...
  def fix_python_header(full_path, relative_path, lines)
    content_start = find_python_content_start(lines)
    content = lines[content_start..].join
  ...
  def find_python_content_start(lines)
    idx = 0
    while idx < lines.length
      line = lines[idx].strip
      # Skip empty lines and single-line path comments at the start
      # Also skip encoding declarations (# -*- coding: utf-8 -*-)
      break unless line.empty? ||
                   (line.start_with?('#') && !line.start_with?('##') && idx == 0) ||
                   (line.start_with?('# -*-') && idx <= 1)
  ...
  def validate_javascript_header(full_path, relative_path)
    lines = File.readlines(full_path)
    return if lines.empty?
    ...
    if errors.any?
      if @fix
        fix_javascript_header(full_path, relative_path, lines)
      else
        @errors << { file: relative_path, issues: errors }
      end
    end
  ...
  def fix_javascript_header(full_path, relative_path, lines)
    content_start = find_javascript_content_start(lines)
    content = lines[content_start..].join
  ...
  def find_javascript_content_start(lines)
    idx = 0
    while idx < lines.length
      line = lines[idx].strip
      # Skip empty lines and single-line path comments at the start
  ...
  def validate_svelte_header(full_path, relative_path)
    lines = File.readlines(full_path)
    return if lines.empty?

    # Expected header:
    # Line 1: <!-- path/to/file.svelte -->
    # Line 2: (blank)
    ...
    if errors.any?
      if @fix
        fix_svelte_header(full_path, relative_path, lines)
      else
        @errors << { file: relative_path, issues: errors }
      end
    end
  ...
  def fix_svelte_header(full_path, relative_path, lines)
    content_start = find_svelte_content_start(lines)
    content = lines[content_start..].join
  ...
  def find_svelte_content_start(lines)
    idx = 0
    while idx < lines.length
      line = lines[idx].strip
      # Skip empty lines and HTML comment headers at the start
