<script>
  ...
  function toggleCt() {
    ...
    updateEndpoint(endpoint._localId, { contentType: next.value });
  }
  ...
  function toggleFieldsMode() {
    if (endpoint.fieldsMode === 'on') {
      ...
      updateEndpoint(endpoint._localId, updates);
    } else {
      // Switching ON
      updateEndpoint(endpoint._localId, { fieldsMode: 'on' });
    }
  }
  ...
  async function runOne() {
    updateEndpoint(endpoint._localId, { state: 'running', result: null });
    ...
    try {
      ...
      updateEndpoint(endpoint._localId, {
  ...
  function remove() {
    removeEndpoint(endpoint._localId);
  }
</script>
